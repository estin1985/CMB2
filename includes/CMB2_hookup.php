...
class CMB2_hookup {
    ...
    public function edit_comment($comment_id, $comment = false)
    {
        ...
        $comment_types = apply_filters('admin_comment_types_dropdown', array( 'comment' => 'Comments' ) );
        ...
        $do_not_pass_go = (
            ...
            || ( ! $comment_type && ! current_user_can( 'moderate_comments', $comment_id ) )
            // check user editing permissions
            || ! current_user_can( 'moderate_comments', $comment_id )
            // get the metabox post_types & compare it to this post_type
            || ! array_key_exists( $comment_type, $comment_types )
        );
